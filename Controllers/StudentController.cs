...
namespace LearningASPMVC.Controllers
{
    public class StudentController : Controller
    {
        ...
        {
            //LINQ from s in db.Students where s.LastName.Contains(SearchBox) || s.FirstName.Contains(SearchBox)
          //("Select * from db.Students s where s.LastName = SearchBox").ToList();
            var students = (from s in db.Students
                            where s.LastName.Contains(SearchBox)
                            || s.FirstName.Contains(SearchBox)
                            select s).ToList();
            return View("Index", students); //call Index page and pass list of students to Index page
        }
